...
import { SOCIAL_PLATFORMS, validatePlatformUrl, detectPlatformFromUrl, getPlatformByKey } from './platformsConfig';
import React from 'react';
import { useFormValue } from 'sanity';

const createSocialLinksArrayField = (includeOfficialWebsite: boolean = false, hideFooterOption: boolean = true) => {

  // Only platforms available for manual selection when auto-detection fails
  const fallbackPlatforms = SOCIAL_PLATFORMS.filter(platform => 
    platform.key === 'officialWebsite' || platform.key === 'genericLink'
  ...
  return defineField({
    ...
    of: [
      {
        ...
        fields: [
          defineField({
            ...
            title: 'URL',
            type: 'url',
            description: 'Paste the complete URL here. The platform will be automatically detected.',
            ...
            components: {
              ...
              field: (props: any) => {
                const DetectedPlatformField = () => {
                  ...
                  const detected = parentValue ? detectPlatformFromUrl(parentValue as string) : null;
    ...
    validation: (Rule) => Rule.custom((items) => {
      ...
      const finalPlatforms = items.map((item) => {
      ...
      const platformsAllowingMultiple = SOCIAL_PLATFORMS
        .filter(p => p.allowMultiple)
        .map(p => p.key);
