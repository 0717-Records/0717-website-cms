...
async function populateAlignmentTest() {
  try {
    // Find the Alignment Test page
    const alignmentTestPage = await client.fetch(
      '*[_type == "page" && title == "Alignment Test"][0]'
    );

    if (!alignmentTestPage) {
      console.error('Alignment Test page not found. Please create it first.');
      return;
    }

    console.log('Found Alignment Test page:', alignmentTestPage._id);
    ...
    // Fifth Page Section with Images and Gallery
    pageContent.push({
      _type: 'pageSection',
      _key: uuidv4(),
          ...
          content: [
            {
              ...
              _key: uuidv4(),
              columns: 2,
              images: [] // Will be empty as requested
            },
            {
              _type: 'imageGallery',
              _key: uuidv4(),
    ...
    // Seventh Page Section with Alignment Testing
    ...
    console.error('Error populating alignment test:', error);
    process.exit(1);
  }
}

// Run the script
populateAlignmentTest();